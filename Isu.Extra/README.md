<aside>
‼️ Реализация переиспользует логику ISU. Так же она поддерживается с ней обратную совместимость, что значит, что использование `IIsuService` отдельно от сервиса ISU не должно нарушать инвариант её логики.
